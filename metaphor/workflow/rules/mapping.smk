...
rule decompress_catalogue:
    ...
    output:
        catalogue=temp("output/mapping/{binning_group}/{binning_group}_contigs_catalogue.fna"),
...
rule create_contigs_index:
    ...
    output:
        catalogue_idx=temp("output/mapping/{binning_group}/{binning_group}_contigs_catalogue.mmi"),
    resources:
        mem_mb=get_max_mb(),
    wildcard_constraints:
        binning_group="cobinning"
...
rule create_genes_index:
    ...
    output:
        catalogue_idx=temp("output/mapping/{binning_group}/{binning_group}_genes_catalogue.mmi"),
    resources:
        mem_mb=get_max_mb(),
    wildcard_constraints:
        binning_group="cobinning"
...
rule index_reads:
    input:
        sort="output/mapping/bam/{binning_group}/{sample}-to-{kind}.sorted.bam",
    output:
        index=temp("output/mapping/bam/{binning_group}/{sample}-to-{kind}.sorted.bam.bai"),
    threads: get_threads_per_task_size("big")
    resources:
        mem_mb=get_mb_per_cores,
    wildcard_constraints:
...
rule flagstat:
    input:
        sort="output/mapping/bam/{binning_group}/{sample}-to-{kind}.sorted.bam",
    output:
        flagstat=temp("output/mapping/bam/{binning_group}/{sample}-to-{kind}.flagstat.txt"),
    threads: get_threads_per_task_size("big")
    resources:
        mem_mb=get_mb_per_cores,
    wildcard_constraints:
