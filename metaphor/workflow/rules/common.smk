...
def get_group_or_sample_file(subworkflow, rule, suffix, add_sample_to_suffix=True, wildcard="{group}"):
    """
    ...
    if config["coassembly"]:
        return str(base_path.joinpath(f"{wildcard}/{wildcard}_{suffix}"))
    else:
        if add_sample_to_suffix:
            suffix = f"{wildcard}_{suffix}"
        return str(base_path.joinpath(f"{wildcard}/{suffix}"))
